...
class Table(Flowable):
    ...
    def normalizeData(self, data):
        ...
        def normCell(stuff):
            if stuff is None:
                return ''
            elif sys.version_info[0] != 3 and isinstance(stuff,unicode):
            ...
            for i in range(nr):
                for j in range(nc):
                    ...
                    if isinstance(v,(list,tuple,Flowable)):
                        ...
                        ix, jx, vx = i, j, v
                        b = (vx and isStrType(v)) and 1 or 0
    ...
    def _drawLines(self):
        ...
        for op, (sc,sr), (ec,er), weight, color, cap, dash, join, count, space in self._linecmds:
            if isStrType(sr) and sr.startswith('split'): continue
            if sc < 0: sc = sc + self._ncols
            if ec < 0: ec = ec + self._ncols
            if sr < 0: sr = sr + self._nrows
            if er < 0: er = er + self._nrows
    ...
    def _splitRows(self,availHeight):
        ...
        for op, (sc,sr), (ec,er), weight, color, cap, dash, join, count, space in self._linecmds:
            if isStrType(sr) and sr.startswith('split'):
